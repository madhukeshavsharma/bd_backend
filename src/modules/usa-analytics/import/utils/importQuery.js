...
function importQuery(validated_req) {
    ...
    const query = {
        ...
        Country: filters && filters.country && filters.country.length
            ...
            : '',
        Country_of_Destination: filters && filters.country_of_destination && filters.country_of_destination.length
            ? { $in: filters.country_of_destination.map(c => new RegExp(escapeRegExp(c), 'i')) }
            : '',
        Port_of_Loading: filters && filters.port_of_loading && filters.port_of_loading.length
            ? { $in: filters.port_of_loading.map(pc => new RegExp(escapeRegExp(pc), 'i')) }
            : '',
